...
[![Deploy to Cloudflare Workers](https://deploy.workers.cloudflare.com/button)](https://deploy.workers.cloudflare.com/?url=https://github.com/chenqs000/Cloudflare-Navihive)
...
5. 在部署界面上，您需要配置以下内容：
    - **项目名称**：为您的导航站项目取个名字
    - **D1 数据库**：点击"创建新数据库"，命名为`navigation-db`(如果通过导入现有库的方式创建程序，部署界面没有创建数据库的设置，需要手动先创建D1数据库然后复制该数据库ID，修改wrangler.jsonc中的数据库ID)
    - **环境变量**：
